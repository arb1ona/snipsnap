...
function ContentNote() {
  ...
  return (
    <div
      className={`  ${
        isMobile ? "w-4/5 mt-[50%] shadow-lg h-[1040px]" : "w-1/2"
      }  p-6 z-30   rounded-lg ${
  ...
  return (
    <div className="flex gap-2  text-[12px]  mt-8">
      <DescriptionOutlinedIcon
        sx={{ fontSize: 18 }}
        className={` mt-[9px] ${
          isHovered ? "text-purple-600" : "text-slate-400"
        }`}
      />
          ...
          <IconButton disabled={isCopied}>
            {isCopied ? (
              <DoneAllOutlinedIcon
                sx={{ fontSize: 18 }}
                className={`${
                  darkMode[1].isSelected ? "text-white" : "text-slate-400"
                }`}
              />
            ) : (
  ...
  function LanguageMenu() {
    ...
    return (
      <div
        ...
        }   h-[220px]  absolute flex-col gap-2 p-3 w-[250px]  rounded-md left-3 bg-slate-100 z-50 flex   text-slate-400`}
      >
        <div
          className={`i${
