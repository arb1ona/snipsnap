...
function TagsWindow() {
  ...
  return (
    <div
      ...
      className={`${openTagsWindow ? "fixed" : "hidden"} ${
        darkMode[1].isSelected ? "bg-slate-800 text-white" : "bg-white"
      }    h-[600px] max-sm:w-[430px] w-[60%] z-40 p-9   shadow-md rounded-md`}
    >
      <Header />
...
function SearchBar({
  ...
  return (
    <div className="flex  gap-5 items-center justify-between mt-11 relative ">
      <div
        className={`h-[42px] flex items-center text-sm  rounded-md  pl-3 gap-1 w-[85%] ${
          darkMode[1].isSelected ? "bg-slate-600" : "bg-slate-50"
        }  `}
      >
...
function TagsList({ searchQuery }: { searchQuery: string }) {
  ...
  return (
    <div
      className={`rounded-md select-none p-3   ${
        darkMode[1].isSelected ? "bg-slate-600" : "bg-slate-50"
